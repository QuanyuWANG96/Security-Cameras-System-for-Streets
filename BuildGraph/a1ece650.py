...
def getStreetInformation(input):
    coordinate = re.findall(r'\(-?\d+,-?\d+\)', input)
    streetName = eval(re.findall(r'\".*\"', input)[0]).lower()
...
def graphPrinter(vertex, edge):
    ...
    for i in range(len(vertex)):
        vertex[i].index = i + 1
        print("  " + str(vertex[i].index) + ":" + " " ,vertex[i])
    print("}")
    ...
    # print "edge_tograph",edge_to_graph
...
def valid(string):
    match = re.match(
        "([ac]\\s+\"[a-zA-Z ]+\"\\s+(\\(\\-?(0|[1-9][0-9]*),\\-?(0|[1-9][0-9]*)\\)\\s*)+)|(r\\s+\"[a-zA-Z ]+\"\\s*)|g{1}",
        string)
    if not match:
        return False
    else:
...
def parse_line(input):
    if len(input) == 0:
        raise Exception("Error: can not find command")
    if valid(input):
        if input[0] == 'a':
            str = getStreetInformation(input)
            if len(str[0]) == 0:
            ...
            str = getStreetInformation(input)
            if len(str[0]) == 0:
                raise Exception('Error: unvalid street name input for \'c\' command')
            if len(str[1]) <= 1:
                raise Exception('Error: unvalid street coordianate input for \'c\' command')
            flag_street = 0
            for i in range(len(strData)):
                if strData[i][0] == str[0]:
                    strData[i][1] = str[1]
                    flag_street = 1
            if flag_street == 0:
        ...
        raise Exception('Error: Invalid Command. Available commands are a,r,c or g')
