    ...
    while (!std::cin.eof()) {
        ...
        if( line[0] == 'V'){
           ...
           edge.clear();
            ...
            if (maxVertex<0){
                std::cout << "Error : can not find vertex"<< std::endl;
            }
            ...
            for (std::vector<int>::size_type i = 0; i<edge.size() ; i++){
                ...
                if (edge[i]>=maxVertex or edge[i] <0){
                    std::cout << "Error : Invalid vertice: "<< edge[i] << std::endl;
                    break;
                }
            }
            ...
            if (maxVertex<0 ){
                std::cout << "Error : can not find graph"<< std::endl;
            }
            adj = getAdj(maxVertex, edge);
            std::vector<int> m = split(res, " ");
            s= m[0];
            v = m[1];
            if (s<maxVertex and s>=0 and v<maxVertex and v>=0){
                ...
                std::cout << "Error : vertex out of bounds"<< std::endl;
            }
        }
    }
    exit(0);
